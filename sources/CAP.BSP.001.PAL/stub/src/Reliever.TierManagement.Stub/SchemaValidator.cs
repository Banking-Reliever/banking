...
/// <summary>
/// Loads the runtime JSON Schema for RVT.BSP.001.FRANCHISSEMENT_ENREGISTRE at startup
...
public sealed class SchemaValidator
{
    ...
    public SchemaValidator(IOptions<StubOptions> options, ILogger<SchemaValidator> logger)
    {
        ...
        _logger.LogInformation(
            "Loaded runtime JSON Schema for RVT.BSP.001.FRANCHISSEMENT_ENREGISTRE from '{Path}' (x-bcm-version={Version}).",
            schemaPath,
            bcmVersion);
    }
    ...
    public void Validate(string payloadJson)
    {
        ...
        if (errors.Count > 0)
        {
            ...
            throw new SchemaValidationException(
                $"Payload does not validate against RVT.BSP.001.FRANCHISSEMENT_ENREGISTRE schema: {errorList}",
                errors);
        }
    }
