# CAP.BSP.001.PAL — Tier Management — Development Stub

This is the **development stub** for `CAP.BSP.001.PAL` (Tier Management). Mode B (contract+stub) per `task_type: contract-stub` of TASK-001.

It publishes simulated `RVT.BSP.001.FRANCHISSEMENT_ENREGISTRE` resource events on the RabbitMQ topic exchange owned by `CAP.BSP.001.PAL` so that consumers (`CAP.CAN.001.TAB`, future `CAP.CAN.001.NOT`, `CAP.B2B.001.CRT`, …) can develop their consumer logic in complete isolation, before the real Tier Management engine is implemented.

The stub is a sibling of the future `sources/CAP.BSP.001.PAL/backend/` (the real microservice). When the engine ships, the stub is decommissioned — consumers do not need to change a line because the contract is identical.
...
| Exchange | `bsp.001.pal-events` (topic, owned by CAP.BSP.001.PAL — Rule 1, 5) |
| Routing key | `EVT.BSP.001.PALIER_HAUSSE.RVT.BSP.001.FRANCHISSEMENT_ENREGISTRE` (Rule 4) |
| Resource event | `RVT.BSP.001.FRANCHISSEMENT_ENREGISTRE` (only this — no autonomous EVT message, Rule 2) |
| Payload form | Domain event DDD — atomic transition data (Rule 3) |
| Schema | `../../plan/CAP.BSP.001.PAL/contracts/RVT.BSP.001.FRANCHISSEMENT_ENREGISTRE.schema.json` |
| sens | Always `FRANCHISSEMENT` (upward only — Epic 1 scope) |
...
1. **Start RabbitMQ** locally:
   ...
   ```bash
   cd sources/CAP.BSP.001.PAL/stub
   docker compose up -d
   ```
...
3. **Subscribe a consumer queue**:
   ...
   - Bind a queue to the exchange `bsp.001.pal-events` with the routing key
     `EVT.BSP.001.PALIER_HAUSSE.RVT.BSP.001.FRANCHISSEMENT_ENREGISTRE`
     (or `EVT.BSP.001.PALIER_HAUSSE.#` to receive every resource event tied to that business event).
...
- Every payload has `sens=FRANCHISSEMENT` (upward-only constraint).
- Every payload has a strictly upward tier transition (no T1→T0 etc.).
- Validator rejects payloads with `sens=RETROGRADATION` (out of scope).
- Validator rejects payloads missing the correlation key `identifiant_dossier`.

## Layout

```
sources/CAP.BSP.001.PAL/stub/
├── README.md                 ← this file
...
│       ├── appsettings.json
│       └── schemas/
│           └── RVT.BSP.001.FRANCHISSEMENT_ENREGISTRE.schema.json
└── tests/
...
When the real `sources/CAP.BSP.001.PAL/backend/` ships its first version of the upward-tier engine, this stub is removed. Consumers do not need to change a line because the contract is identical.
