...
///
/// Inactive by default (StubOptions.Active=false). Activate via env var STUB_Stub__Active=true
...
public sealed class Worker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                var (payloadJson, caseId, allocationId) = _payloadFactory.BuildNext(rotation++);
                ...
                _logger.LogInformation(
                    "Published RVT.BSP.004.CONSUMPTION_RECORDED for case={CaseId} alloc={AllocationId}",
                    caseId, allocationId);
            }
