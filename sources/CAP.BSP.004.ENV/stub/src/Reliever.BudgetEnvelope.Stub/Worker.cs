...
/// <c>RVT.BSP.004.CONSUMPTION_RECORDED</c> payloads on the RabbitMQ
/// topic exchange owned by CAP.BSP.004.ENV.
///
...
public sealed class Worker : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _logger.LogInformation(
            "Stub active — exchange={Exchange} routingKey={RoutingKey} cadence={Cadence}/min ({PeriodMs}ms) cases={Cases}",
            _stubOptions.ExchangeName, _stubOptions.RoutingKey, _stubOptions.CadencePerMinute, periodMs,
            _stubOptions.SimulatedCases.Count);
    ...
    private void OpenChannel()
    {
        ...
        _connection = factory.CreateConnection($"cap-bsp-004-env-stub@{Environment.MachineName}");
        _channel = _connection.CreateModel();

        // Topic exchange owned by CAP.BSP.004.ENV (Rule 1). Durable so it
    ...
    private void Publish(JsonObject payload)
    {
        ...
        _logger.LogInformation(
            "Published RVT.BSP.004.CONSUMPTION_RECORDED on routingKey={RoutingKey} case={Case} category={Category} consumed={Consumed}/{Cap}",
            _stubOptions.RoutingKey,
            payload["case_id"]?.GetValue<string>(),
            payload["category"]?.GetValue<string>(),
            payload["consumed_amount"]?.GetValue<double>(),
            payload["cap_amount"]?.GetValue<double>());
    }
}
