# Task Board — 2026-05-16

> Generated by /sort-task — refresh with `/sort-task`, launch tasks with `/launch-task`

## 🔵 In Progress

| Task | Capability | Title | Epic |
|------|-----------|-------|------|

_No tasks in progress_

---

## 🟡 Awaiting Merge (PR open)

| Task | Capability | Title | PR |
|------|-----------|-------|----|
| TASK-003 `full-microservice` | CAP.SUP.002.BEN | Update anchor PII with sticky-field semantics | [#16](https://github.com/Banking-Reliever/banking/pull/16) |

---

## 🟢 Ready to Start (by priority)

| # | Task | Capability | Title | Priority | Score | Unblocks |
|---|------|-----------|-------|----------|-------|---------|
| 1 | TASK-004 `full-microservice` | CAP.SUP.002.BEN | Anchor lifecycle: archive and restore | medium | 22 | TASK-005, TASK-006 |

> Same capability as the in-review TASK-003 — auto-launch skips it until the capability frees (PR merge OR an explicit one-active-task override).

---

## 🟠 Awaiting Additional Information

> These tasks contain unresolved open questions (`- [ ]` in `## Open Questions`).
> They cannot be launched until these questions are clarified.

| Task | Capability | Title | Open Questions |
|------|-----------|-------|----------------|
| TASK-003 `full-microservice` | CAP.BSP.001.SCO | Continuous score recomputation (Flow B) | 3 pending (AUT readiness, SIG readiness, tier-thresholds cache invalidation) |
| TASK-004 `full-microservice` | CAP.BSP.001.SCO | Entry-score baseline (Flow A) | 2 pending (HARD BLOCKER on `CAP.BSP.002.ENR`; foundation readiness) |
| TASK-006 `full-microservice` | CAP.BSP.001.SCO | Model versioning, operability hardening, and contract harness | 1 pending (recomputation throughput NFRs) |
| TASK-002 | CAP.CHN.001.DSH | Subscription point and consumption layer for CAP.CHN.001.DSH | 1 pending |
| TASK-004 | CAP.CHN.001.DSH | BSP.004.AUT stub and transaction history web view | 2 pending |
| TASK-005 | CAP.CHN.001.DSH | Mobile view — nomadic dashboard consultation | 2 pending |
| TASK-006 | CAP.CHN.001.DSH | Consumer-side validation against the real CORE event stream | 2 pending |

---

## ⚫ Stalled (loop budget exhausted — human required)

| Task | Capability | Title | Loops used | Last failing criteria |
|------|-----------|-------|------------|----------------------|

_No stalled tasks_

---

## 🔴 Blocked

| Task | Capability | Title | Blocked By |
|------|-----------|-------|------------|
| TASK-005 `full-microservice` | CAP.BSP.001.SCO | Query surface and event-sourced read-models | TASK-003 |
| TASK-003 | CAP.CHN.001.DSH | Consent gate and current situation web view | TASK-002 |
| TASK-005 `full-microservice` | CAP.SUP.002.BEN | GDPR Art. 17 pseudonymisation at the anchor | TASK-002 (done), TASK-003 (in_review), TASK-004 (ready) |
| TASK-006 `full-microservice` | CAP.SUP.002.BEN | PII-free anchor history projection and audit-trail query | TASK-002 (done), TASK-003 (in_review), TASK-004 (ready), TASK-005 |

---

## ✅ Done

| Task | Capability | Title |
|------|-----------|-------|
| TASK-001 | CAP.BSP.001.SCO | Contract and development stub for `EVT.BSP.001.SCORE_RECALCULE` ([PR #2](https://github.com/Banking-Reliever/banking/pull/2)) |
| TASK-002 `contract-stub` | CAP.BSP.001.SCO | Python rewrite — stub publisher for all 3 RVTs ([PR #10](https://github.com/Banking-Reliever/banking/pull/10)) |
| TASK-001 | CAP.BSP.001.TIE | Contract and development stub for `EVT.BSP.001.TIER_UPGRADED` ([PR #1](https://github.com/Banking-Reliever/banking/pull/1)) |
| TASK-001 | CAP.BSP.004.ENV | Contract and development stub for `EVT.BSP.004.ENVELOPE_CONSUMED` ([PR #6](https://github.com/Banking-Reliever/banking/pull/6)) |
| TASK-001 `contract-stub` | CAP.SUP.002.BEN | Contract and development stub for `RVT.SUP.002.BENEFICIARY_ANCHOR_UPDATED` ([PR #9](https://github.com/Banking-Reliever/banking/pull/9)) |
| TASK-002 `full-microservice` | CAP.SUP.002.BEN | Foundation: mint anchor (UUIDv7) + synchronous GET ([PR #15](https://github.com/Banking-Reliever/banking/pull/15)) |

---

## Critical Path

```
CAP.SUP.002.BEN:
  TASK-001 ✅ → TASK-002 ✅
                    ├─ TASK-003 🟡 in_review (PR #16) — UPDATE with sticky-PII
                    └─ TASK-004 🟢 ready — ARCHIVE/RESTORE — held by one-active-task rule
                                             └─ TASK-005 🔴 PSEUDONYMISE (depends on 003+004)
                                                              └─ TASK-006 🔴 HISTORY
```

**Awaiting merge:** PR #16. Once it merges, `CAP.SUP.002.BEN/TASK-004` automatically becomes the next eligible candidate on this capability.

**Next to refine elsewhere:**
- `CAP.BSP.001.SCO/TASK-003` (Flow B) — 3 OQs gated on AUT/SIG upstream + tier-cache policy
- `CAP.BSP.001.SCO/TASK-004` (Flow A) — HARD BLOCKER on `CAP.BSP.002.ENR`
- `CAP.CHN.001.DSH/TASK-002` (BEN identity resolution) — unlocks the CHN chain
