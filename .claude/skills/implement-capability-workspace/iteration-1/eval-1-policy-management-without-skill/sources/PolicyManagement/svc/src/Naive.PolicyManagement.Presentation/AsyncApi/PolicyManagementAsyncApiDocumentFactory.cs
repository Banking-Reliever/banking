...
namespace Naive.PolicyManagement.Presentation.AsyncApi;
...
public static class PolicyManagementAsyncApiDocumentFactory
{
    public static AsyncApiDocument Create()
    {
        var policyCreatedMessage = CreateMessage(
            messageId: "EVT.RES.NAIVE.POLICY.POLICY_CREATED",
        ...
        var policySuspendedMessage = CreateMessage(
            messageId: "EVT.RES.NAIVE.POLICY.POLICY_SUSPENDED",
        ...
        {
            Id = "urn:naive:capability:PolicyManagement",
            DefaultContentType = "application/json",
            Info = new AsyncApiInfo
            {
                Title = "Naive.PolicyManagement Async API",
                Version = "1.0.0",
                Description = "Async API contract for the Naive PolicyManagement capability. " +
                              "This capability publishes PolicyCreated and PolicySuspended events " +
                              "on the naive-policy-management-channel."
            },
            Channels = new Dictionary<string, AsyncApiChannel>
            {
                ["naive-policy-management-channel/PolicyCreated"] = new AsyncApiChannel
                {
                    Description = "Outgoing event: a new insurance policy has been created.",
                    Publish = new AsyncApiOperation
                    {
                        OperationId = "EVT.RES.NAIVE.POLICY.POLICY_CREATED",
                ...
                ["naive-policy-management-channel/PolicySuspended"] = new AsyncApiChannel
                {
                    Description = "Outgoing event: an active insurance policy has been suspended.",
                    Publish = new AsyncApiOperation
                    {
                        OperationId = "EVT.RES.NAIVE.POLICY.POLICY_SUSPENDED",
